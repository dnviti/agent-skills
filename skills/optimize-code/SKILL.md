...
> prices before pinning them to phases.
...
2. **Map the code with a read-only exploration pass.** Find candidate hot
   ...
   keep only the summary. Do **not** record anything derivable from
   `grep`/`git` as memory.
...
- **Floor tier** — codebase mapping, greps, parsing profiler/bench output,
  summarizing noisy logs, mechanical formatting.
- **Mid tier** — standard optimizations, adding/adjusting benchmarks, most
  refactors that follow a clear profile.
- **High tier** — tricky concurrency/cache/layout issues, non-obvious
  algorithmic changes, security-sensitive hot paths, standard adversarial
  review.
- **Top tier** — orchestration, ambiguous architecture-level performance
  design, adversarial review of critical logic.

**Relative token cost, cheapest first:** Floor ≪ Mid < High (~2× Mid) < Top
(~2× High). Most work should sit at the bottom two tiers.
...
phases; reserve high effort for hard-reasoning optimize phases and adversarial
review.
...
Keep entries short. **Prune before you add.**
...
- Memory is context, not enforcement: if a rule **must** hold (never touch
  prod, never commit secrets, never disable tests to "pass" a bench), enforce
  it with a **mechanical guardrail** and say so explicitly.
...
pass. Feed fixes back and **re-test / re-bench**.
...
bottleneck, scope shifts): **stop.** Record what changed and why in
`./.work/PLAN.md` (and project memory if lasting), re-plan the affected
phases, and continue.
